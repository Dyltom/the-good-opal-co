...
export const checkoutFormSchema = z.object({
  ...
  phone: z.string().regex(AUSTRALIAN_PHONE_REGEX, 'Please enter a valid Australian mobile number'),
  ...
  // Additional options
  giftMessage: z.string().max(200, 'Gift message must be less than 200 characters').optional(),
...
export const customJewelrySchema = z.object({
  ...
  customType: z.string().max(50, 'Custom type must be less than 50 characters').optional(),
  ...
  timeline: z
    .enum(['asap', '1-month', '2-months', '3-months', 'flexible'])
  ...
  // Style preferences
  stylePreferences: z
    .array(z.enum(['classic', 'modern', 'bohemian', 'minimalist', 'vintage', 'bold']))
    .min(1, 'Please select at least one style preference'),

  // Metal preferences
  metalPreference: z
    .enum(['silver', 'gold', 'rose-gold', 'white-gold', 'mixed'])
    .describe('Please select a metal preference'),

  // Stone preferences
  opalType: z
    .array(z.enum(['black', 'white', 'crystal', 'boulder', 'fire', 'any']))
