...
describe('sold ring reference proportions', () => {
  test.each(ringStyles.map(({ id }) => id))(
    ...
    (style) => {
      ...
      expect(innerWallClearanceMm).toBeGreaterThanOrEqual(-0.02)
      expect(innerWallClearanceMm).toBeLessThanOrEqual(0.05)
      expect(lipOverlapMm).toBeGreaterThanOrEqual(0.05)
      expect(lipOverlapMm).toBeLessThanOrEqual(0.1)
    }
  )
})
